...
export default async function PlanPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return (
    <main className="mx-auto mt-8 max-w-7xl space-y-8">
      ...
      <Card className="p-7 hover:translate-y-0">
        ...
        <PlanGenerateForm defaultDate={exam.date.toISOString().slice(0, 10)} />
      </Card>
            ...
            <article key={plan.id} className="ose-card relative p-6">
              <div className="flex items-start justify-between gap-4">
                <Link href={`/plan/${plan.id}`} className="min-w-0 flex-1">
                  <span className="rounded-full bg-primary-soft px-3 py-1 text-xs font-black text-primary">{plan.status}</span>
                  <h2 className="mt-4 text-2xl font-black text-navy">{plan.title}</h2>
                  <p className="mt-2 font-semibold text-muted">目标日期 {plan.targetExamDate.toISOString().slice(0, 10)} · {completed}/{plan.days.length} 天已完成</p>
                </Link>
                <PlanDeleteButton planId={plan.id} className="shrink-0" />
              </div>
              <Link href={`/plan/${plan.id}`} aria-label={`查看${plan.title}`}>
                <div className="mt-4 h-3 overflow-hidden rounded-full bg-orange-100" role="progressbar" aria-valuenow={Math.round(ratio)} aria-valuemin={0} aria-valuemax={100}>
      ...
      {totalPages > 1 ? (
        <nav className="flex justify-center gap-3" aria-label="分页">
          {page > 1 ? <Link className="rounded-2xl bg-white px-4 py-3 font-black text-navy shadow-soft" href={`/plan?page=${page - 1}`}>上一页</Link> : null}
          <span className="rounded-2xl bg-white px-4 py-3 font-black text-muted">{page}/{totalPages}</span>
          {page < totalPages ? <Link className="rounded-2xl bg-white px-4 py-3 font-black text-navy shadow-soft" href={`/plan?page=${page + 1}`}>下一页</Link> : null}
        </nav>
      ) : null}
    </main>
  );
