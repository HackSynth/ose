...
export default async function TopicPracticePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const topic = await prisma.knowledgePoint.findUnique({ where: { id }, include: { children: { orderBy: { sortOrder: "asc" } } } });
  if (!topic) notFound();
  ...
  return (
    <main className="mx-auto mt-8 max-w-6xl space-y-6">
      <Button asChild variant="secondary"><Link href="/practice"><ArrowLeft className="h-4 w-4" />返回练习入口</Link></Button>
      <Card className="p-8">
        <p className="mb-3 text-sm font-black text-primary">Topic Practice</p>
        <h1 className="text-4xl font-black text-navy">{topic.name}</h1>
        <p className="mt-3 font-semibold text-muted">共 {questions.length} 道题，覆盖 {topic.children.length || 1} 个子知识点。</p>
        <div className="mt-6"><StartPracticeButton payload={{ mode: "topic", topicId: id, limit: questions.length || 20 }}>开始本知识点练习</StartPracticeButton></div>
      </Card>
        ...
        {questions.map((question) => (
          <Card key={question.id} className="p-5 hover:translate-y-0">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="mb-2 text-sm font-black text-muted">2023 上午 · 第 {question.questionNumber} 题 · {question.knowledgePoint.parent?.name ?? question.knowledgePoint.name} / {question.knowledgePoint.name}</p>
