...
export function AIAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([{ role: "assistant", content: "你好，我是 OSE 智能助手。可以问我软考知识点、刷题方法或备考规划。" }]);
  ...
  async function send() {
    ...
    setInput("");
    const nextMessages = [...messages, { role: "user" as const, content: text }].slice(-AI_CHAT_MAX_MESSAGES);
    ...
    try {
      ...
      if (!response.ok || !response.body) {
        ...
        setMessages((prev) => prev.map((message, index) =>
          index === prev.length - 1 ? { ...message, content: data.message || "AI 服务暂时不可用，请稍后再试。" } : message,
        ));
        return;
      }
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50">
      {open ? (
        <div className="mb-4 flex h-[500px] w-[calc(100vw-2.5rem)] flex-col rounded-[1.5rem] border border-orange-100 bg-warm shadow-lift md:w-[380px]">
          <div className="flex items-center justify-between rounded-t-[1.5rem] bg-white/90 p-4">
            <div>
              ...
              <p className="text-xs font-bold text-muted">{status.configured ? `当前供应商：${status.provider}` : "未配置 AI API Key"}</p>
            </div>
            <button onClick={() => setOpen(false)} aria-label="关闭助手" className="rounded-full p-2 hover:bg-primary-soft">
              <X className="h-5 w-5" />
            </button>
          </div>
          <div className="flex-1 space-y-3 overflow-y-auto p-4" aria-live="polite" aria-atomic="false">
            {messages.map((message, index) => (
              <div key={index} className={cn("flex", message.role === "user" ? "justify-end" : "justify-start")}>
                <div className={cn("max-w-[85%] rounded-2xl p-3 text-sm shadow-sm", message.role === "user" ? "bg-primary-soft text-navy" : "bg-white text-navy")}>
          ...
          <div className="border-t border-orange-100 bg-white/80 p-3">
            <div className="flex gap-2">
              ...
              {loading ? (
                ...
                <Button type="button" onClick={send} disabled={!status.configured} aria-label="发送消息">
