...
export function PlanGenerateForm({ defaultDate }: { defaultDate: string }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
        ...
        <div className="mt-3 flex flex-wrap gap-3">
          {preferences.map((item) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-black ${selected.includes(item) ? 'bg-primary text-white' : 'bg-white text-muted'}`}
            >
              {item}
            </button>
          ))}
      ...
      {error ? (
        <p className="rounded-2xl bg-red-50 px-4 py-3 text-sm font-black text-red-600">{error}</p>
      ) : null}
      <Button size="lg" disabled={loading}>
        {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : null}生成学习计划
      </Button>
    </form>
  );
}
