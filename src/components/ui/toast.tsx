...
const ToastViewport = ({ className, ...props }: React.ComponentPropsWithoutRef<typeof ToastPrimitives.Viewport>) => (
  <ToastPrimitives.Viewport className={cn("fixed bottom-4 right-4 z-[100] flex max-h-screen w-full max-w-sm flex-col gap-2", className)} {...props} />
);
const Toast = ({ className, ...props }: React.ComponentPropsWithoutRef<typeof ToastPrimitives.Root>) => (
  <ToastPrimitives.Root className={cn("rounded-2xl border border-orange-100 bg-white p-4 text-navy shadow-lift", className)} {...props} />
);
