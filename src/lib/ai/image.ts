...
async function getImageConfigSource(userId?: string | null) {
  ...
  const hasImageApiKey = Boolean(
    resolveSecret(settings?.imageApiKeyEncrypted, settings?.imageApiKey)
  );
  const hasRequired = provider === 'custom' ? Boolean(settings?.imageBaseUrl) : hasImageApiKey;
