...
export function buildGenerateChoiceUserMessage(params: { count: number; knowledgePoints: string; difficulty: number; existingQuestionSummaries: string }) {
...
export function buildExplainUserMessage(question: ExplainQuestion, userAnswerLabel: string, isCorrect: boolean) {
...
type GradeSubQuestion = { subNumber: number; content: string; referenceAnswer: string; score: number };

export function buildGradeCaseUserMessage(scenario: GradeScenario, subQuestions: GradeSubQuestion[], userAnswers: Record<string, string>) {
  return `## 案例背景
${scenario.background}

## 各子题及作答
${subQuestions.map((subQuestion) => `### 子题 ${subQuestion.subNumber}（${subQuestion.score}分）
题目：${subQuestion.content}
参考答案：${subQuestion.referenceAnswer}
学生作答：${userAnswers[String(subQuestion.subNumber)] ?? ""}`).join("\n\n")}`;
}
...
export function buildDiagnosisUserMessage(stats: {
  overview: { totalQuestions: number; overallAccuracy: number; streak: number; daysToExam: number; recentDailyAvg: number; recentAccuracy: number; regularity: string; wrongCount: number; unmasteredCount: number };
...
export function buildStudyPlanUserMessage(params: { targetDate: string; daysLeft: number; dailyTime: string; preferences: string[]; overallMastery: number; knowledgeStats: string; weakPoints: string }) {
  return `## 备考条件
