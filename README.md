...
- 前端 UI 重构（持续进行）：已完成主布局统一、题库页组件化拆分、通用 Page 结构组件接入、关键编辑弹窗校验统一，并在统计/练习/模型服务等页面持续收敛表单栅格、移动端替代展示、详情作答卡片与页面级状态反馈（加载/错误/空态）
- 前端 UI 重构（持续进行）：已完成主布局统一、题库页组件化拆分、通用 Page 结构组件接入、关键编辑弹窗校验统一，并在统计/练习/模型服务等页面持续收敛表单栅格、移动端替代展示、详情作答卡片与页面级状态反馈（加载/错误/空态）；验收项见 `docs/ui-acceptance-checklist.md`
